...
//                // Возвращается intent, если пользователь действительно добавил активность
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if (item.getItemId()==R.id.item_filter){
            ...
            FilterDialogFragment filterDialogFragment = new FilterDialogFragment(authors, genres, uncheckedAuthors, uncheckedGenres);
            FragmentManager manager = getSupportFragmentManager();
            FragmentTransaction transaction = manager.beginTransaction();
...
//                finish();
    ...
    @Override
    public void onFilterClick(ArrayList<String> uncheckedAuthors, ArrayList<String> uncheckedGenres) {
        this.uncheckedAuthors=uncheckedAuthors;
        this.uncheckedGenres=uncheckedGenres;
        for (int i = 0; i < notes.size(); i++){
            if (notes.get(i).getItemType()==0){
                if (uncheckedAuthors.contains(((RealNote)notes.get(i)).getAuthor()) ||
                        uncheckedGenres.contains(((RealNote)notes.get(i)).getGenre())){
                    notes.get(i).setVisibility(false);
//                    layoutManager.findViewByPosition(i).setVisibility(View.GONE);

                }
                else{
//                    layoutManager.findViewByPosition(i).setVisibility(View.VISIBLE);
                    ...
                    .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                        ...
                        public void onSuccess(final DocumentSnapshot documentSnapshot0) {
                            ...
                            final RealNote realNote = new RealNote(id, map.get("path").toString(), map.get("author").toString(),
                                    ...
                                    (double) map.get("publicRatingSum"), (long)map.get("publicRatingCount"), map.get("genre").toString());
